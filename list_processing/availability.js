...
a list of transactions. The function should return true only if
the sum of the quantity values of the item's transactions is greater
than zero. Notice that there is a movement property in each transaction
...
function transactionsFor(inventoryItem, object) {
  return object.filter(inventory => inventory.id === inventoryItem);
}

function isItemAvailable(key, object) {
  let item = transactionsFor(key, object);
  let totalQuantity = item.reduce((accum, val) => {
    if (val.movement === 'in') {
